<div align="center">
    ...
    <p><strong>记录一些导航程序编译调试过程中遇到的问题，和我找到的解决方案</strong></p>
</div>
<div align="center">
...
> 解决办法有两种：
>
> * 遇到标准库函数的语句，用单步跳过，而不是单步跳入，已经进入库函数了就单步跳出；
>
> * 改编译器配置，一劳永逸的解决问题
>
>   * **VS**：改 default.natstepfilter 文件【我没亲自试过】：
>
>     ```bash
...
> 安装 Qt5.15.2，下载速度只有二十k，实在太慢，以前我安装 Qt 6 的时候还没这么慢。且此版本开始无离线安装包，只能在线安装。
>
...
### 004-Windows 下打开（尤其是Qt）软件显示缺少 “xxx.dll” 文件

> * 去 dll 网站（比如：找dll）下载缺失的 .dll 文件；放到 `C:\Windows\SysWOW64` 或 `C:\Windows\System32` 下面。
...
### 007-调试 C/C++ 程序出现异常，调试直接进汇编代码，不知道是哪里的问题

> * 大部分编译器都可以显示调用堆栈，可以直接让我们看到执行哪一行代码出了问题；
> * 常见的异常可能是使用了没初始化的变量字段、访问已经释放的指针；

---

### 008-使用 Eigen 库编译时报一堆错，不知道是哪里的问题

> * Eigen 是 C++ 的一个开源线性代数库，主要是进行矩阵运算，对导航算法相当关键，要知道导航算法基本都是矩阵计算。除了基本的矩阵计算之外，支持四元数、旋转矩阵，C++ 写的导航定位的开源代码中基本都是用 Eigen。
...
> * 提示 command not found 可能是因为文件没有执行权限，可以添加：`chmod +x <.sh 脚本名>`；
...
>
> * 先去网站下载插件：https://marketplace.visualstudio.com/，后缀名：VXIS
> * 把 VXIS 文件放到 WSL 中，VScode 中可以直接把资源管理器中的文件托入左侧文件列表。
> * 选择通过 VXIS 安装，在下来菜单中选择要安装的 VXIS 文件，稍等片刻即安装成功。

---

### 016-远程控制 ROS 没有访问权限

> * 远程控制 ROS 需要从机和主机在同一个局域网下面，也就是连同一个 WiFi 或者热点；
> * 改 .bashrc 文件
